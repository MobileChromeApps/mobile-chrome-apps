...
chromespec.registerSubPage('chrome.idle', function(rootEl) {
  ...
  addButton('Change detection interval to 10 seconds', function() {
    ...
    chromespec.log('Detection interval set to 10 seconds.');
  });

  addButton('Change detection interval to 60 seconds', function() {
    chrome.idle.setDetectionInterval(60);
    chromespec.log('Detection interval set to 60 seconds.');
  });

  // Add a status-change listener.
  var stateListener = function(state) {
    chromespec.log('State changed: ' + state);
  };
